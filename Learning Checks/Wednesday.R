...
bb <- contrast(m.emm, 'trt.vs.ctrl', ref='female0') %>%
  broom::tidy() 
# %>%
  # head() %>%
  # pander()
...
##deviance test (use refit=TRUE since fixed effects are not the same)
?anova
anova(m1, m2, refit = TRUE)
...
m3 <- lmer(Support ~ 1 + factor(female) + 
             age_cwc + age_mean + 
             factor(Religion) + (1|Districts), 
           data = data, REML = TRUE)
summary(m3)
