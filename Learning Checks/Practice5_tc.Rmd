...
library(pacman)
p_load("utils", "lme4", "lmerTest", "sjPlot", "tidyverse", install = T)
...
```{r}
# Transform data here
canines <- read.csv("../Practice Data/phlebitis.csv") %>%
...
model1 <- lmer(Y ~ 1 + (1|Animal), data=canines, REML=FALSE)
summary(model1)
# as_tibble
VarCorr(model1) %>%
  as_tibble() %>%
...
Enter level 2 fixed effect for Treatment and level 1 fixed effect for Time. 

```{r}
# Create conditional model with new fixed effects
model2 <- lmer(Y ~ 1 + Treatment + factor(Time) + (1|Animal), data=canines, REML=FALSE)
summary(model2)
```
...
```{r, message = F}
# Interaction plot
model3 <- lmer(Y ~ 1 + Treatment + factor(Time) + Treatment:factor(Time) + (1|Animal), data=canines, REML=FALSE)
summary(model3)
```
...
Three pairs of the same images ("Cues") are randomized to appear on the screen that features a threat on either the right half of the computer screen or the left half of the computer screen. On the other half of the screen is a non-threatening image. Threatening images involved graphics of a masked robber pointing a gun at the participant, an attacking mountain lion, or a snake lunging toward the participant. Participants were given a three-second countdown in between each cue.
