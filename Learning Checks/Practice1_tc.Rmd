...
library(tidyverse)
?read_xlsx
nerds <- readxl::read_xlsx("../Practice Data/NE_EdSpending_Jan28th_22.xlsx", na="NA") %>%
  mutate(level = as.factor(level), 
         enroll_c = enroll_raw_NE - mean(enroll_raw_NE, na.rm=TRUE))
# %>%
  # mutate(across(ends_with("NE"), ~as.numeric(.x)))
str(as.factor(nerds$level))
summary(nerds$enroll_c)
str(as.factor(nerds$district_ID))
?across()
?required
?read_excel()
```
...
model1 <- lm(pp_total_raw_NE ~ enroll_raw_NE + level, data = nerds)
summary(model1)  
...
If the interaction *is* significant, interpret it by answering whether or not the effect of school enrollment on total expenditures per-pupil depends on grade level. 

```{r}
# Enter multiple regression model with interaction here
model2 <- lm(pp_total_raw_NE ~ enroll_c*level, data = nerds)
summary(model2)  
